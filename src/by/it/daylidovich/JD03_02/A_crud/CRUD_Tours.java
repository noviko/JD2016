...
public class CRUD_Tours {
    ...
    public static void create(int FK_country, int FK_typeTour, int FK_transport, int FK_typeHotel, int FK_foodComplex, int cost, int discount) throws SQLException{
        Connection connection  = DriverManager.getConnection(ADB.URL_DB, ADB.USER_DB, ADB.PASSWORD_DB);
        Statement statement = connection.createStatement();
        statement.executeUpdate("INSERT INTO tours(FK_country, FK_type_tour, FK_transport, FK_type_hotel, FK_food_complex, Cost, Discount) VALUES (" + FK_country + ", " + FK_typeTour + ", " + FK_transport + ", " + FK_typeHotel + ", " + FK_foodComplex + ", " + cost + ", " + discount  + ")");
        ResultSet resultSet = statement.executeQuery("SELECT * FROM tours ORDER BY ID DESC LIMIT 1");
    ...
    public static void update(int id,int FK_country, int FK_typeTour, int FK_transport, int FK_typeHotel, int FK_foodComplex, int cost, int discount) throws SQLException{
        Connection connection  = DriverManager.getConnection(ADB.URL_DB, ADB.USER_DB, ADB.PASSWORD_DB);
        Statement statement = connection.createStatement();
        statement.executeUpdate("UPDATE tours SET FK_country=" + FK_country + ",  FK_type_tour=" + FK_typeTour + ",  FK_transport=" + FK_transport + ",  FK_type_hotel=" + FK_typeHotel + ",  FK_food_complex=" + FK_foodComplex + ",  Cost=" + cost + ", Discount="+ discount +" WHERE id=" + id);
        ResultSet resultSet=statement.executeQuery("SELECT * FROM tours WHERE id=" + id);
    ...
    public static void delete(int id) throws SQLException{
        Connection connection  = DriverManager.getConnection(ADB.URL_DB, ADB.USER_DB, ADB.PASSWORD_DB);
        Statement statement = connection.createStatement();
        statement.executeUpdate("DELETE FROM tours WHERE id=" + id);
        read();
        connection.close();
    }
}
